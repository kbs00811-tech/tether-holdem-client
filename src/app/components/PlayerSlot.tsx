...
interface PlayerSlotProps {
  ...
  onSitDown?: () => void;
  hideCards?: boolean;
  isDealingNow?: boolean;      // V16: 딜링 애니메이션 재생 중 → opponent cards 숨김
  // V3 P2C: 쇼다운 시 서버 CARDS_SHOWN 이벤트로 공개된 상대 카드
  shownCards?: Array<{ suit: "spades" | "hearts" | "diamonds" | "clubs"; rank: string }>;
  isWinner?: boolean;          // V3 P2C: 승자 하이라이트용
  onTopUp?: () => void;
  onEmoji?: () => void;
  onSitOut?: () => void;
}
  ...
  return (
    <div className="flex flex-col items-center gap-0 relative">
      ...
      {/* Opponent face-down cards — floating above
          데스크탑: md (54×76) — hero 카드에 가까운 시각적 무게로 부각
          대형 데스크탑: lg (72×101) — 카드 디테일까지 보임
          모바일: xs (28×40) 그대로 — 좁은 화면 보호 */}
      <AnimatePresence>
        {showOpponentCards && !isAllIn && (
          <motion.div
            ...
            style={{
              // V20: 모든 좌석 — 카드를 아바타 위(뒤)에 배치 (GG포커 스타일)
              ...((() => {
                return {
                  top: isLargeDesktop ? -8 : isDesktop ? -4 : -1,
                  left: '50%',
      ...
      {/* V3 P2C: Winner glow 링 — 승자 슬롯에 금색 펄스 */}
      {isWinner && !isDead && (
        <motion.div
          className="absolute pointer-events-none"
          style={{
            top: -14, left: "50%",
            width: isDesktop ? 130 : isMobile ? 80 : 100,
            height: isDesktop ? 130 : isMobile ? 80 : 100,
            transform: "translateX(-50%)",
            borderRadius: "50%",
            background: "radial-gradient(circle, rgba(255,215,0,0.55) 0%, rgba(255,215,0,0.2) 40%, transparent 70%)",
            zIndex: -1,
          }}
